...
class SpatialDataset():
    ...
    def load_or_compute_adata(self) -> ad.AnnData:
        ...
        if (not os.path.exists(os.path.join(self.dataset_path, f'adata.h5ad'))) or self.force_compute:
            ...
            # raw_adata = ad.read_h5ad(os.path.join(self.dataset_path, f'adata_raw.h5ad'))
            # expressed_gene_index = raw_adata.X.sum(axis=0).astype(bool).flatten().nonzero()[1]
            # expressed_gene_mat = raw_adata.X[:, expressed_gene_index]
        
            # pro_missing_frac = 1-(collection_processed.layers['mask'].sum().sum()/(collection_processed.n_vars*collection_processed.n_obs))
            # pro_corrupt_spot_frac = 1-(collection_processed.layers['mask'].all(axis=0).sum()/collection_processed.n_obs)
            # raw_missing_frac = (1-((raw_adata.X!=0).sum()/(raw_adata.X.shape[0]*raw_adata.X.shape[1])))
            # raw_missing_frac_expressed_genes = (1-((expressed_gene_mat!=0).sum()/(expressed_gene_mat.shape[0]*expressed_gene_mat.shape[1])))
            # print('Missing values in processed data: {:.3f}%'.format(100*pro_missing_frac))
            # print('Corrupt spot fraction in processed data: {:.3f}%'.format(100*pro_corrupt_spot_frac))
            # print('Missing values in raw data (excluding zero genes): {:.3f}%'.format(100*raw_missing_frac_expressed_genes))
            # print('Missing values in raw data (including zero genes): {:.3f}%'.format(100*raw_missing_frac))

            # if not os.path.exists('missing_values_stats.txt'):
            #     with open("missing_values_stats.txt", "w") as text_file:
            #         text_file.write("dataset,processed,raw,raw_non_zero,corrupt_spot\n")
            
            # with open("missing_values_stats.txt", "a") as text_file:
            #         text_file.write(f"{self.dataset},{pro_missing_frac},{raw_missing_frac},{raw_missing_frac_expressed_genes},{pro_corrupt_spot_frac}\n")
            ###
    ...
    def get_graphs(self, n_hops: int, layer: str) -> dict:
        ...
        def get_graphs_one_slide(self, adata: ad.AnnData, n_hops: int, layer: str, hex_geometry: bool) -> Tuple[dict,int]:
            ...
            # Get the used mean key depending on the layer
            if layer == 'noisy_d':
                used_mean_key = 'c_d_log1p_avg_exp'
            elif 'deltas' in layer:
                used_mean_key = f'{layer}_avg_exp'.replace('deltas', 'log1p')
            else:
                used_mean_key = None

            # Add a used_mean attribute to the graph if the layer is for delta training
            used_mean = torch.from_numpy(self.adata.var[used_mean_key].values).type(torch.float32) if used_mean_key != None else None
    ...
    def get_pretrain_dataloaders(self, layer: str = 'c_d_log1p', batch_size: int = 128, shuffle: bool = True, use_cuda: bool = False) -> Tuple[AnnLoader, AnnLoader, AnnLoader]:
        ...
        if layer == 'noisy':
            ...
            c_t_log1p = self.adata.layers['c_t_log1p'].copy()
            # Get zero mask
            zero_mask = ~self.adata.layers['mask']
            # Zero out the missing values
            c_t_log1p[zero_mask] = 0
            # Add the layer to the adata
            self.adata.layers['noisy'] = c_t_log1p
        ...
        if layer == 'noisy_d':
            ...
            gene_means = self.adata.var['c_t_log1p_avg_exp'].values
            ...
            noisy_deltas[valid_mask] = self.adata.layers['c_t_deltas'][valid_mask]
            # Add the layer to the adata
        ...
        ### FIXME: Addition to handle noisy training (this is temporal and should be in process_dataset) ###
        if layer == 'noisy_d':
            # Add a var column of used means of the layer
            mean_key = f'c_t_log1p_avg_exp'
            self.adata.var['used_mean'] = self.adata.var[mean_key]
        ### End of addition ###
    ...
    def get_predictions(self, model, layer: str = 'c_d_log1p', batch_size: int = 128, use_cuda: bool = False)->None:
        ...
        with torch.no_grad():
            ...
            if 'deltas' in layer:
                means = torch.tensor(self.adata.var['used_mean'], device=glob_expression_pred.device)
